...
Atendiendo a la ecuación \ref{eq:Verhulst} podemos ver que sigue un crecimiento exponencial. Para limitar este crecimiento tan desmedido, Verhulst asumió que el ratio $R$ varía con el tamaño de la poblacion. Así postuló que la variación de R respecto al tamaño de la poblacion es proporcional al a $1-x_n$, es decir,
\[R = r(1-x_n)\]
donde la constante r será llamada \textbf{parámetro de crecimiento}.

Teniendo en cuenta esta nueva relación podemos escribir
\[x_{n+1}=(1+R)x_n = (1+r)x_n-rx_n^2\]

Podemos observar que esta ecuación tiene dos puntos fijos, es decir, hay dos valores posibles para $x_0$ que hacen que la ecuación se mantenga en valor constante. Estos valores son $x_0=0$ y $x_0=1$.

Si tenemos $x_0=0$ nos encontramos ante una población vacía que, evidentemente no puede crecer ni decrecer. Sin embargo, si tomamos un valor distinto de 0 y positivo, por muy pequeño que sea, tendremos $r>0$ lo que dará lugar a un crecimiento de la población.

El caso de $x=1$ merece ser estudiado en profundidad.

\subsubsection{Proceso de Verhulst para $x_0=1$}
Para estudiar esta situación analizaremos cómo se comportan pequeñas desviaciones de $x_n$ a lo largo del tiempo. Para ello definimos la variable $d_n$ que representará una desviación en el valor de $x_n$. Atendiendo a la ecuación \ref{eq:Verhulst} podemos ver que esta desviación se propaga de la forma:
\[d_{n+1} = (1+R)d_n\]

Podemos observar que para valores de $R$ entre $0$ y $-2$, la desviación decrece en valor absoluto con respecto a la desviación en el paso anterior. Para cualquier otro valor de $R$ podemos observar que esta desviación va siendo cada vez mayor.
...
\[A=\{(\pm 1, 0)\}\]
\end{example}
Volviendo a los sistemas disipativos, estudiemos el caso del movimiento de un péndulo. La primera de las figuras muestra el comportamiento de un péndulo simple sobre el que no actúa la fuerza de la gravedad, mientras que la segunda figura muestra el resultado de aplicarle dicha fuerza que amortigüa su movimiento haciéndolo cesar.
